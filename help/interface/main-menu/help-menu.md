---
helpx_url: 'https://helpx.adobe.com/substance-3d-painter/interface/main-menu/help-menu.html'
breadcrumb-title: ''
description: 請進入 Substance 3D Painter 的說明選單，找到文件、教學及支援資源。
helpx_creative_field: ''
helpx_description: Painter > Interface > Main menu > Help menu
helpx_experience_level: ''
helpx_learn_topic: ''
helpx_tags: ''
title: 說明選單
user-guide-description: ''
user-guide-title: ''
source-git-commit: e370ba212d3e90f71e09b75ff41be6123d37c5eb
workflow-type: tm+mt
source-wordcount: '279'
ht-degree: 1%

---


# 說明選單

<table style="table-layout:auto">
  <tr style="border: 0;">
    <td style="border: 0;" valign="top">說明選單包含連結和選項，讓你能進一步了解 Substance 3D Painter。 這包括教學、發行說明和文件等有用內容，或是你可以解決 Painter 本地安裝問題的地方，包括存取日誌或回報錯誤的方式。</td>
    <td style="border: 0;" valign="top"><img src="../../assets/help-menu.png" alt=""/></td>
  </tr>
</table>

| 動作 | 說明 |
| --- | --- |
| 教學課程 | 官方相關教學](https://helpx.adobe.com/substance-3d/unlisted/tutorials.html)連結[。 |
| 發行說明 | 發行](../../release-notes/all-changes.md)說明連結[。 |
| 文件 | 連結至此文件。 |
| 捷徑列表 | 捷徑](../settings/shortcuts.md)文件連結[。 |
| 腳本文件 | 連結到各種腳本 API 的本地文件。 |
| 論壇 | 申請](https://community.adobe.com/t5/substance-3d-painter/bd-p/substance-3d-painter)論壇連結[。 |
| 回報錯誤 | 打開錯誤回報視窗來傳送資訊。 |
| 節目記錄 | 在預設的 TXT 檔案編輯器中開啟你目前 Painter 會話的日誌檔案。 |
| 輸出日誌 | 匯出日誌檔，請求支援協助。 |
| 給予回饋 | 連結到我們的 [功能請求](https://feedback.substance3d.adobe.com/forums/261284-substance-3d-painter) 平台。 |
| 管理我的帳戶 | 開啟 [account.adobe.com](http://account.adobe.com) 來管理你的帳號和訂閱。 |
| 登出 | 請登出目前已登入的 Adobe 帳號。 |
| 歡迎畫面 | 打開 [歡迎屏風](../../getting-started/getting-started.md) 。 |
| 有什麼新鮮事 | 顯示「What&#39;s new」畫面，了解更多目前更新的變更資訊。 |
| 主畫面 | 打開主畫面。 |
| 請留意最新消息 | 打開 [更新檢查](../miscellaneous/update-checker.md) 視窗。 |
| 關於Substance 3D Painter | 開啟「關於我們」視窗，顯示應用程式版本、法律通知及其他細節。 |
| 合作夥伴 | 存取整合於 Painter 的合作夥伴軟體資訊。 |
